...
const CHAT_HISTORY_KEY_PREFIX = "cooragent.chatHistory.v1";

mermaid.initialize({ startOnLoad: false, theme: "default" });

const getChatHistoryKey = (userId) =>
...
const persistChatHistory = (userId, entries) => {
  try {
    localStorage.setItem(
      getChatHistoryKey(userId),
      JSON.stringify(entries.slice(0, CHAT_HISTORY_LIMIT))
    );
    return true;
  } catch (err) {
...
const renderChatHistory = () => {
  if (!conversationHistoryList || !conversationHistoryMeta) return;
  ...
  if (!userId) {
    ...
    empty.textContent = "暂无可显示的用户对话";
    conversationHistoryList.appendChild(empty);
    return;
  }

  const entries = loadChatHistory(userId);
  conversationHistoryMeta.textContent = `${userId} · 已保存 ${entries.length}/${CHAT_HISTORY_LIMIT} 条`;
  clearChatHistoryBtn.disabled = entries.length === 0;

  if (!entries.length) {
    const empty = document.createElement("div");
    empty.className = "conversation-history-empty";
    empty.textContent = "该用户还没有对话记录";
    conversationHistoryList.appendChild(empty);
    return;
  }
  ...
  entries.forEach((entry, index) => {
    ...
    item.setAttribute("aria-label", `加载第 ${index + 1} 条历史消息`);

    const header = document.createElement("span");
    header.className = "conversation-history-item-header";
    ...
    timestamp.textContent = formatChatHistoryTime(entry.createdAt);
...
const addChatHistoryEntry = (userId, content) => {
  const entries = loadChatHistory(userId);
  entries.unshift({
  ...
  persistChatHistory(userId, entries);
  renderChatHistory();
};
...
const runWorkflow = async () => {
  ...
  addChatHistoryEntry(userId, message);
...
userIdInput.addEventListener("input", renderChatHistory);
