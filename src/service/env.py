...
MEMORY_LONG_TERM_TOKEN_BUDGET = max(
    0, _parse_int("MEMORY_LONG_TERM_TOKEN_BUDGET", 512)
)
MEMORY_MAX_RECORD_CHARS = _parse_int("MEMORY_MAX_RECORD_CHARS", 8000)
MEMORY_RECENT_TURNS = min(2, max(0, _parse_int("MEMORY_RECENT_TURNS", 2)))
MEMORY_AUTO_CONSOLIDATION_ENABLED = _parse_bool(
    "MEMORY_AUTO_CONSOLIDATION_ENABLED", True
)
MEMORY_CONSOLIDATION_LLM_ENABLED = _parse_bool(
    "MEMORY_CONSOLIDATION_LLM_ENABLED", True
)
MEMORY_MARKDOWN_PROJECTION_ENABLED = _parse_bool(
    "MEMORY_MARKDOWN_PROJECTION_ENABLED", True
)
...
# Legacy whole-workflow Skill storage remains readable, but new automation is retired.
WORKFLOW_SKILL_ENABLED = _parse_bool("WORKFLOW_SKILL_ENABLED", False)
WORKFLOW_SKILL_REUSE_ENABLED = _parse_bool("WORKFLOW_SKILL_REUSE_ENABLED", False)
WORKFLOW_SKILL_AUTO_DISTILL_ENABLED = _parse_bool("WORKFLOW_SKILL_AUTO_DISTILL_ENABLED", False)
...
# Reuse starts in shadow mode while reflection and candidate distillation collect evidence.
...
AGENT_SKILL_REFLECTION_ENABLED = _parse_bool(
    "AGENT_SKILL_REFLECTION_ENABLED", True
)
AGENT_SKILL_REFLECTION_MIN_CONFIDENCE = _parse_float(
    "AGENT_SKILL_REFLECTION_MIN_CONFIDENCE", 0.75
)
AGENT_SKILL_REFLECTION_TIMEOUT_SECONDS = max(
    1.0, _parse_float("AGENT_SKILL_REFLECTION_TIMEOUT_SECONDS", 60.0)
)
